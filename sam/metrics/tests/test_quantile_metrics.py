...
from sam.metrics import compute_quantile_ratios, compute_quantile_crossings
import pandas as pd
import numpy as np
...
class TestQuantileMetrics(unittest.TestCase):
    ...
    def test_quantile_crossings(self):
        ...
        expected_keys = ['0.800 < 0.600', '0.600 < mean',
                         'mean < 0.400', '0.400 < 0.200']
        ...
        self.assertRaises(AssertionError, compute_quantile_crossings, pred, 0, [0.5, 'mean'])

        pred = pd.DataFrame({
            'predict_lead_0_q_0.5': [6, 2, 4, 5],
            'predict_lead_0_mean': [3, 1, 3, 6]})
