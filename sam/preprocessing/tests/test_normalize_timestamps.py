...
class TestCompleteTimestamps(unittest.TestCase):
    ...
    def test_fillna_method(self):
        ...
        result = normalize_timestamps(data, '15min', start_time,
                                      end_time, fillna_method='bfill')
    ...
    def test_empty_start_end_time(self):
        ...
        output = pd.DataFrame({
            "TIME": pd.to_datetime(
                ['2018/01/01 16:00:00', '2018/01/01 16:15:00',
                 '2018/01/01 16:30:00']),
            "ID": 1,
            "TYPE": 2,
            "VALUE": [1, 3, 4]
    ...
    def test_multiple_ids(self):
        ...
        output = pd.DataFrame({
            "TIME": pd.to_datetime(
                ['2018/01/01 16:00:00', '2018/01/01 16:15:00',
                 '2018/01/01 16:30:00', '2018/01/01 16:00:00',
                 '2018/01/01 16:15:00', '2018/01/01 16:30:00']),
            "ID": [1, 1, 1, 2, 2, 2],
            "TYPE": 2,
    ...
    def test_round_method(self):
        ...
        output = pd.DataFrame({
            "TIME": pd.to_datetime(
                ['2018/01/01 15:45:00', '2018/01/01 16:00:00', '2018/01/01 16:15:00',
                 '2018/01/01 15:45:00', '2018/01/01 16:00:00', '2018/01/01 16:15:00']),
            "ID": 1,
            "TYPE": [1, 1, 1, 2, 2, 2],
            "VALUE": [1, 2, np.NaN, 3, np.NaN, 4]
    ...
    def test_incorrect_input(self):
        ...
        self.assertRaises(ValueError, normalize_timestamps, data,
                          'half uur', start_time, end_time)
        # wrong is not a time
        # integers are actually allowed, they are interpreted as UNIX time
        self.assertRaises(ValueError, normalize_timestamps, data,
                          '15min', 'wrong', end_time)
        ...
        self.assertRaises(Exception, normalize_timestamps, data, '15min',
                          round_function='flotor')
        self.assertRaises(Exception, normalize_timestamps, data, '15min',
                          fillna_method='supersmartfill')
        self.assertRaises(Exception, normalize_timestamps,
                          data.assign(ID=np.nan), '15min')
        self.assertRaises(Exception, normalize_timestamps,
                          data.assign(TYPE=np.nan), '15min')
        self.assertRaises(Exception, normalize_timestamps,
                          data.iloc[[]], '15min')
