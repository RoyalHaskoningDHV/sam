...
def sam_format_to_wide(data, sep='_'):
    ...
    data_wide: dataframe
        ...
        TIME column. For example, if ID is 'abc' and TYPES are 'debiet' and 'stand', the created
        column names will be 'abc_debiet' and 'abc_stand', as well as 'TIME'. The result will be
...
def wide_to_sam_format(data, sep='_', idvalue='', timecol='TIME'):
    ...
    is given by timecol. Furthermore, the TYPE/ID combinations should be present in value column
    names that look like ID(sep)TYPE.

    If sep is None, then all column names (except timecol) are assumed to be TYPE, with no id,
    so id will always be set to `idvalue`.
