...
class TestIncidentHeatmap(unittest.TestCase):
    ...
    def test_incident_heatmap(self):
        ...
        ts = pd.DataFrame(
            {"incident": np.tile([0.1, 0.2, 0.3], 15), "id": np.tile(["A", "B", "C"], 15)},
            index=range,
            columns=["incident", "id"],
        )
        ...
        ax = plot_incident_heatmap(
            ts, resolution="W", annot=True, cmap="Reds", figsize=(15, 8), datefmt="%Y-%m-%d"
        )
    ...
    def test_incident_heatmap_advanced(self):
        ...
        ax = plot_incident_heatmap(
            ts, resolution="row", normalize=True, cmap=pal, datefmt="%Y-%m-%d", figsize=(15, 9)
        )
        return ax.get_figure()
