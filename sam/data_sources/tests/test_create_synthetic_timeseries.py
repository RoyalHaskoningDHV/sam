...
class TestCreateSyntheticTimeseries(unittest.TestCase):
    ...
    def test_nonoise(self):
        result = create_synthetic_timeseries(self.dates)
        expected = np.array([0, 0, 0, 0], dtype=np.float64)
        assert_array_equal(result, expected)

    def test_some_incorrect_inputs(self):
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates, minmax_values=0)
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates, cutoff_values=True)
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates, monthly=None)
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates, daily='1')
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates, hourly=['1'])
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates, monthnoise=1)
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates,
                          daynoise=('normal', 'a'))
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates, noise=1)
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates, negabs='1')
        self.assertRaises(Exception, create_synthetic_timeseries, self.dates, random_missing=2)
        # There must be at least 2 datetimes to create a spline
        empty_dates = pd.Series([], dtype='datetime64[ns]')
        self.assertRaises(Exception, create_synthetic_timeseries, empty_dates)
        empty_dates = pd.Series(['2016-01-01 00:00:00'], dtype='datetime64[ns]')
        self.assertRaises(Exception, create_synthetic_timeseries, empty_dates)

    def test_noise(self):
        without_noise = create_synthetic_timeseries(self.many_dates, 1, 2, 3)
        with_noise = create_synthetic_timeseries(self.many_dates, 1, 2, 3,
                                                 noise={'normal': 2, 'poisson': 1})
        self.assertGreater(np.var(with_noise), np.var(without_noise))

    def test_minmax(self):
        result = create_synthetic_timeseries(self.many_dates, 1, 2, 3,
                                             noise={'normal': 2, 'poisson': 1},
                                             minmax_values=(0, 10))
    ...
    def test_negabs(self):
        ...
        without_negabs = create_synthetic_timeseries(self.many_dates, 1, 2, 3,
                                                     noise={'normal': 2, 'poisson': 1},
                                                     minmax_values=(0, 10))
        with_negabs = create_synthetic_timeseries(self.many_dates, 1, 2, 3,
                                                  noise={'normal': 2, 'poisson': 1},
    ...
    def test_random_missing(self):
        result = create_synthetic_timeseries(self.many_dates, 1, 2, 3, random_missing=0.5)
    ...
    def test_seed(self):
        ...
        foo = create_synthetic_timeseries(self.many_dates, 1, 2, 3, ("normal", 4), ("poisson", 3),
                                          {"normal": 2, "poisson": 3}, seed=42)
        bar = create_synthetic_timeseries(self.many_dates, 1, 2, 3, ("normal", 4), ("poisson", 3),
                                          {"normal": 2, "poisson": 3}, seed=42)
